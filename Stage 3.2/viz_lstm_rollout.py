...
def main():
    ...
    ap.add_argument("--run_dir", required=True, help="Path to a saved run folder under runs/")
    ap.add_argument("--out", default=None, help="Optional output video path (e.g., runs/.../viz/out.mp4)")
    ap.add_argument("--fps", type=int, default=12)
    ap.add_argument("--dpi", type=int, default=120)
    ap.add_argument("--limit_objects", type=int, default=None, help="Render only first K validation objects")
    ap.add_argument("--t_from", type=int, default=None, help="Render from this global time (inclusive)")
    ap.add_argument("--t_to", type=int, default=None, help="Render until this global time (inclusive)")
    args = ap.parse_args()
    ...
    out_path = Path(args.out) if args.out else (viz_dir / "rollout_val.mp4")

    # Render
    render_video(run_dir, out_path, fps=args.fps, dpi=args.dpi,
                 XY=XY, frame_index=frame_index, object_ids=object_ids,
                 val_ids=val_ids, model=model, device=device, window=window,
                 norm_mean=norm_mean, norm_std=norm_std,
                 limit_objects=args.limit_objects, t_from=args.t_from, t_to=args.t_to)
