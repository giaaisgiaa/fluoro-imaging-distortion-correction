...
def main():
    ...
    # Ask for validation percentage
    while True:
        try:
            val_input = input("\nEnter validation percentage (default is 10): ").strip()
            if val_input == "":
                args.val_frac = 0.1  # default 10%
                break
            val_percent = float(val_input)
            if 0 < val_percent <= 100:
                args.val_frac = val_percent / 100.0
                break
            print("Please enter a number between 0 and 100")
        except ValueError:
            print("Please enter a valid number")
    
    print(f"\nUsing {args.val_frac*100:.1f}% of objects for validation")
