...
from non_stationary_models.forward_polynomial import PolyDisplacementModel
...
def evaluate_rollouts(
    ...
    if verbose:
        ...
        plt.title(f"Validation RMSE over time (Polynomial degree={model.degree})")
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        # Save the plot
        plt.savefig(f'visualizations/polynomial/rmse_over_time_degree{model.degree}.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def main():
    ...
    model = PolyDisplacementModel(degree=3).fit(X_train, Y_train)

    # --- Evaluate ---
    overall_rmse, per_t_rmse, avg_rmse, per_t_std = evaluate_rollouts(D, val_idx, model, verbose=True)

    print("\n=== Validation metrics (Polynomial degree=3) ===")
