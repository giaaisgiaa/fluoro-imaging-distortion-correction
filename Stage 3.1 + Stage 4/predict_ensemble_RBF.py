...
def evaluate_model(target_frame=7, validation_split=0.2):
    """
    Evaluate the RBF ensemble model on a specific target frame.
    """
    print("="*60)
    print(f"ENSEMBLE RBF PREDICTION vs REAL COORDINATES")
    ...
    model.fit_tensor(training_data, t0=0.0, dt=1.0)
...
def compute_average_performance(model, training_data, validation_data):
    ...
    plt.title("Validation RMSE over time (Ensemble RBF)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    
    # Save plot
    os.makedirs("visualizations/ensemble_rbf", exist_ok=True)
    plot_filename = "visualizations/ensemble_rbf/rmse_over_time.png"
